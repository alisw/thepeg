// -*- C++ -*-
//
// TheP8EventShapes.cc is a part of ThePEG - Toolkit for HEP Event Generation
...
// functions of the TheP8EventShapes class.
//
// This code has been copied and adapted from the corresponding
...
NoPIOClassDescription<TheP8EventShapes> TheP8EventShapes::initTheP8EventShapes;
// Definition of the static class description member.

void TheP8EventShapes::Init() {

  static ClassDocumentation<TheP8EventShapes> documentation
    ("There is no documentation for the TheP8EventShapes class");

}

void TheP8EventShapes::calcHemisphereMasses() {
...
vector<double> TheP8EventShapes::eigenvalues(const double T[3][3]) {
  ...
  if (4.*p*sqr(p) + 27.*sqr(q) > 2.0e-16) {
    ...
    cerr << "TheP8EventShapes::eigenvalues: found D = "
	 << 4.*p*sqr(p) + 27.*sqr(q) 
...
Axis TheP8EventShapes::eigenvector(const double T[3][3], const double &lam) {
...
void TheP8EventShapes::diagonalizeTensors(bool linear, bool cmboost) {  
  // initialize
  double Theta[3][3];
  for(int i=0; i<3; i++) {
    for(int j=0; j<3; j++) {
...
void TheP8EventShapes::calculateThrust() { 
...
void TheP8EventShapes::calcT(const vector<Momentum3> &p, Energy2 &t, Axis &taxis) {
  Energy2 tval;
  t = ZERO;
...
void TheP8EventShapes::calcM(const vector<Momentum3> &p, Energy2 &m, Axis &maxis) {
  Energy2 mval;
  m = ZERO;
  Momentum3 tv, ptot;
  vector<Momentum3> cpm;
...
void TheP8EventShapes::bookEEC(vector<double> & hi) {
