...
void LEPEventShapes::analyze(const tPVector & particles) {
  ...
  _omthr ->fill( 1.-_shapes->thrust() ,eventweight);
  _maj ->fill( _shapes->thrustMajor() ,eventweight);
  _min ->fill( _shapes->thrustMinor() ,eventweight);
  _obl ->fill( _shapes->oblateness() ,eventweight); 
  _c ->fill( _shapes->CParameter() ,eventweight); 
  _d ->fill( _shapes->DParameter() ,eventweight); 
  _sph ->fill( _shapes->sphericity() ,eventweight);
  _apl ->fill( _shapes->aplanarity() ,eventweight);
  _pla ->fill( _shapes->planarity() ,eventweight); 
  _mhi ->fill( _shapes->Mhigh2() ,eventweight);
  _mlo ->fill( _shapes->Mlow2() ,eventweight); 
  _mdiff ->fill( _shapes->Mdiff2() ,eventweight); 
  _bmax ->fill( _shapes->Bmax() ,eventweight); 
  _bmin ->fill( _shapes->Bmin() ,eventweight); 
  _bsum ->fill( _shapes->Bsum() ,eventweight); 
  _bdiff ->fill( _shapes->Bdiff() ,eventweight); 
}

void LEPEventShapes::dofinish() {
  AnalysisHandler::dofinish();
  if ( !checkHistogramFactory() ) return;
  unitNormalize(_omthr);
  unitNormalize(_maj);
  unitNormalize(_min);
  unitNormalize(_obl);
  unitNormalize(_sph);
  ...
  unitNormalize(_mhi);
  unitNormalize(_mlo);
  unitNormalize(_mdiff);
  unitNormalize(_bmax);
  unitNormalize(_bmin);
  unitNormalize(_bsum);
  unitNormalize(_bdiff);
}

void LEPEventShapes::dataSet(const std::string & path,
    const std::string & title, const std::vector<double> & bins,
    const std::vector<double> & y, const std::vector<double> & yerr){
  std::vector<double> x;
  for(std::vector<double>::const_iterator it = bins.begin();
      it + 1 != bins.end(); it++){
    x.push_back((*it + *(it+1)) / 2.0);
  }
  std::vector<double> xerr(x.size(), 0.0);
  histogramFactory().dataSetFactory().createXY(path, title, 
      x, y, xerr, yerr);
}
...
void LEPEventShapes::doinitrun() {
  ...
  histogramFactory().mkdirs("/LEPEventShapes");
  histogramFactory().cd("/LEPEventShapes");
  ...
  _omthr = histogramFactory().createHistogram1D
    ("omthr", "One Minus Thrust", bins);
  dataSet("omthr_data", "DELPHI data", bins, data, error);
  // major
  ...
  _maj = histogramFactory().createHistogram1D
    ("maj", "Thrust Major", bins);
  dataSet("maj_data", "DELPHI data", bins, data, error);
  // minor
  ...
  _min = histogramFactory().createHistogram1D
    ("min", "Thrust Minor", bins);
  dataSet("min_data", "DELPHI data", bins, data, error);
  // oblateness
  ...
  _obl = histogramFactory().createHistogram1D
    ("obl", "Oblateness", bins);
  dataSet("obl_data", "DELPHI data", bins, data, error);
  // sphericity
  ...
  _sph = histogramFactory().createHistogram1D
    ("sph", "Sphericity", bins);
  dataSet("sph_data", "DELPHI data", bins, data, error);
  // aplanarity
  ...
  _apl = histogramFactory().createHistogram1D
    ("apl", "Aplanarity", bins);
  dataSet("apl_data", "DELPHI data", bins, data, error);
  // planarity
  ...
  _pla = histogramFactory().createHistogram1D
    ("pla", "Planarity", bins);
  dataSet("pla_data", "DELPHI data", bins, data, error);
  // C
  ...
  _c = histogramFactory().createHistogram1D
    ("c", "C parameter", bins);
  dataSet("c_data", "DELPHI data", bins, data, error);
  // D
  ...
  _d = histogramFactory().createHistogram1D
    ("d", "D parameter", bins);
  dataSet("d_data", "DELPHI data", bins, data, error);
  // M high
  ...
  _mhi = histogramFactory().createHistogram1D
    ("mhi", "High hemisphere mass", bins);
  dataSet("mhi_data", "DELPHI data", bins, data, error);
  // M low
  ...
  _mlo = histogramFactory().createHistogram1D
    ("mlo", "Low hemisphere mass", bins);
  dataSet("mlo_data", "DELPHI data", bins, data, error);
  // M diff
  ...
  _mdiff = histogramFactory().createHistogram1D
    ("mdiff", "Difference in hemisphere masses", bins);
  dataSet("mdiff_data", "DELPHI data", bins, data, error);
  // Bmax
  ...
  _bmax = histogramFactory().createHistogram1D
    ("bmax", "Wide jet broadening measure", bins);
  dataSet("bmax_data", "DELPHI data", bins, data, error);
  // Bmin
  ...
  _bmin = histogramFactory().createHistogram1D
    ("bmin", "Narrow jet broadening measure", bins);
  dataSet("bmin_data", "DELPHI data", bins, data, error);
  // Bsum
  ...
  _bsum = histogramFactory().createHistogram1D
    ("bsum", "Sum of jet broadening measures", bins);
  dataSet("bsum_data", "DELPHI data", bins, data, error);
  // Bdiff
  ...
  _bdiff = histogramFactory().createHistogram1D
    ("bdiff", "Difference of jet broadening measures", bins);
  dataSet("bdiff_data", "DELPHI data", bins, data, error);
}
